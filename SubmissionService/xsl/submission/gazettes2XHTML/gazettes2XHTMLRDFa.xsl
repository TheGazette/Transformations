...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:html="http://www.w3.org/1999/xhtml" xmlns:wlf="http://www.williamslea.com/xsl/functions"
  xmlns:t="urn:templates"
  xmlns="http://www.w3.org/1999/xhtml">

  <xsl:import href="owlClasses.xsl"/>
  <xsl:output method="xml" omit-xml-declaration="no" indent="no" exclude-result-prefixes="#all"
    encoding="utf-8"/>
    ...
    <xsl:choose>
      ...
      <xsl:when test="$noticeCode='1125'">
        <xsl:choose>
          ...
          <xsl:when test="normalize-space(//gz:Notice/gz:Honour)='C.V.O.'"></xsl:when>
      ...
      <xsl:when test="$noticeCode='1129'">
        <xsl:choose>
          ...
          <!--this value is not provided into the documentation. needs to be updated later-->
          <xsl:when test="normalize-space(//gz:Notice/gz:Honour)=''">KnightGrandCrossOrderOfTheBritishEmpire</xsl:when>
  ...
  <xsl:variable name="medal">
    <xsl:choose>
      ...
      <xsl:when test="$noticeCode='1131'">RoyalRedCross</xsl:when>
  ...
  <xsl:function name="wlf:name-sibling">
    ...
    <xsl:variable name="thiscount">
      <xsl:value-of
        select="concat('-',count($source/preceding-sibling::*[name()=$source/name()]) + 1)"/>
    </xsl:variable>
  ...
  <xsl:function name="wlf:compound-name">
    ...
    <xsl:variable name="compound"
      select="concat(wlf:name-sibling($first),concat('-',wlf:name-sibling-no-namespace($second)))"/>
  ...
  <xsl:function name="wlf:triple-compound-name">
    ...
    <xsl:variable name="compound"
      select="concat(wlf:name-sibling($first),concat('-',concat(wlf:name-sibling-no-namespace($second),concat('-',wlf:name-sibling-no-namespace($third)))))"/>
    <xsl:value-of select="$compound"/>
  </xsl:function>
  ...
  <xsl:function name="wlf:contains" as="xs:boolean">
    ...
    <xsl:variable name="temp" as="xs:boolean*">
      <xsl:for-each select="$list">
        <xsl:if
          test="contains(translate($str, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), .)">
  ...
  <xsl:variable name="paramConfigXml"
    select="if (doc-available('LGconfiguration.xml')) then doc('LGconfiguration.xml') else ()"/>
  <xsl:variable name="gaz">https://www.thegazette.co.uk/</xsl:variable>
  <xsl:variable name="osPostcode" as="xs:string*"
    >http://data.ordnancesurvey.co.uk/ontology/postcode/</xsl:variable>
  <xsl:variable name="osPostcodeUnit" as="xs:string*"
    >http://data.ordnancesurvey.co.uk/id/postcodeunit/</xsl:variable>
  <xsl:variable name="noticeType" select="//gz:Notice/@Type"/>
  <xsl:variable name="noticeNo">
    <xsl:choose>
      <xsl:when
        test="//gz:Notice/@Reference and (starts-with($noticeId,'L') or starts-with($noticeId,'B') or starts-with($noticeId,'E'))">
  ...
  <xsl:variable name="publishDate">
    <xsl:variable name="test"
      select="concat(.//ukm:PublishDate,'T',(if (//ukm:TimeStamp != '') then wlf:getTimeStamp(//ukm:TimeStamp) else '01:00:00'))"/>
  ...
  <xsl:variable name="issueURI"
    select="concat($gaz,'id/','edition/',$edition,'/issue/',$issueNumber)"/>
  ...
  <xsl:variable name="long-notice-uri">http://www.<xsl:value-of select="lower-case($edition)"
      />-gazette.co.uk/id/issues/<xsl:value-of select="$issueNumber"/>/notices/<xsl:value-of
      select="$noticeNo"/>
  </xsl:variable>
  ...
  <xsl:variable name="notice-provenance">https://www.thegazette.co.uk/id/notice/<xsl:value-of
      select="$noticeId"/>/provenance</xsl:variable>
  <xsl:variable name="trustee-act-1925-part-2-section-27-uri"
    >http://www.legislation.gov.uk/ukpga/Geo5/15-16/19/section/27</xsl:variable>

  <!-- ######################## -->
  <!-- ##### ASSOCIATIONS ##### -->
  <!-- ######################## -->
  <xsl:variable name="applies-bankruptcy-restrictions"
    >gzw:appliesBankruptcyRestrictions</xsl:variable>
  ...
  <xsl:variable name="has-principal-trading-address"
    >gazorg:hasPrincipalTradingAddress</xsl:variable>
  ...
  <xsl:variable name="recognizes-petition-bankruptcy-order-date"
    >gzw:petitionBankruptcyOrderDate</xsl:variable>
  <xsl:variable name="recognizes-petition-filing-date">insolvency:datePetitionFiled</xsl:variable>
  <xsl:variable name="recognizes-vol-winding-up-resolution-date"
    >insolvency:dateResolvedVoluntaryWindingUp</xsl:variable>
    ...
    <html
      ...
      <body>
        <xsl:if test="$debug='true'">
          ...
          <h1>DEBUG START</h1>paramconfigxml: |<xsl:copy-of
            select="$paramConfigXml/gzc:Configuration//gzc:Notice[@Code = $noticeCode]"/>|<h1>DEBUG
            END</h1>
        </xsl:if>
        <xsl:apply-templates/>
      </body>
  ...
  <xsl:template name="content">
    ...
    <div class="content" about="{$notifiable-thing}" data-gazettes="Notice"
  ...
  <xsl:template match="*:ul[@data-created-bullet='true']" mode="finesse">
    <xsl:if
      test="not(local-name(preceding-sibling::*[1]) = 'ul' and  preceding-sibling::*[1][@data-created-bullet='true'])">
      <xsl:copy>
        <xsl:for-each select="@*">
        ...
        <xsl:apply-templates select="node()" mode="finesse"/>
        <xsl:if
          test="local-name(following-sibling::*[1]) = 'ul' and following-sibling::*[1][@data-created-bullet='true']">
  ...
  <xsl:template match="*:ul[@data-created-bullet='true']" mode="finesse2">
    <xsl:apply-templates select="node()" mode="finesse"/>
    <xsl:if
      test="local-name(following-sibling::*[1]) = 'ul' and following-sibling::*:ul[@data-created-bullet='true']">
      <xsl:apply-templates select="following-sibling::*[1]" mode="finesse2"/>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="rdfa-data">
    ...
    <div class="rdfa-data">
      <span about="{$noticeURI}" property="{$has-publisher}"
      ...
      <span about="{$noticeURI}" property="{$has-notice-number}" datatype="xsd:integer"
        content="{$noticeNo}"/>
      <xsl:if test="$issueNumber">
        ...
        <span about="{$issueURI}" property="{$has-issue-number}" datatype="xsd:string"
          content="{$issueNumber}"/>
        <span about="{$noticeURI}" property="{$is-the-same-as}"
          resource="https://www.thegazette.co.uk/id/edition/{$edition}/issue/{$issueNumber}/notice/{$noticeNo}"
          typeof="{$notice-type}"/>
        <span about="{$noticeURI}" property="{$is-an-alternate-of}" resource="{$long-notice-uri}"
          typeof="{$notice-type}"/>
      </xsl:if>
      ...
      <span resource="this:notifiableThing"
        typeof="https://www.thegazette.co.uk/def/publication#NotifiableThing"/>
      </xsl:if>
      ...
      <xsl:if test="$noticeCode='1122' or $noticeCode='1123' or $noticeCode='1124' or $noticeCode='1125' or $noticeCode='1128' or $noticeCode='1129'"> 
        ...
        <span about="this:notifiableThing" property="honours:isAppointedAs" resource="this:honour-1"></span>
        <span about="this:honour-1" property="rdf:type" resource="honours:{wlf:stripSpecialChars($honour)}"></span>
        <xsl:choose>
          <xsl:when test="$noticeCode =('1123','1124','1125','1126','1127','1128','1129','1130')">
            <span about="this:honour-1" property="honours:isMemberOfOrder" resource="honours:{$order}"></span>
          </xsl:when>
          <xsl:when test="$noticeCode='1124'">
            <span about="this:honour-1" property="honours:isMemberOfOrder" resource="honours:TheOrderOfStMichaelAndStGeorge"></span>
          </xsl:when>
        </xsl:choose>       
        <span about="this:honour-1" property="honours:isAwardedTo" resource="this:person-1"></span>
      </xsl:if>      
      <!---Medal-->
      <xsl:if test="$noticeCode='1127' or $noticeCode='1130' or $noticeCode='1131' or $noticeCode='1132' or $noticeCode='1133' or $noticeCode='1134' or $noticeCode='1135' or  $noticeCode='1136'"> 
        <xsl:comment>helper</xsl:comment>
        <span about="this:notifiableThing" property="honours:isAwarded" resource="this:medal-1"></span>
        <span about="this:medal-1" property="rdf:type" resource="honours:{wlf:stripSpecialChars($medal)}"></span>
        <xsl:if test="$noticeCode =('1127','1128','1129','1130')">
          <span about="this:honour-1" property="honours:isMemberOfOrder" resource="honours:{$order}"></span>
        </xsl:if>
        <span about="this:medal-1" property="honours:isAwardedTo" resource="this:person-1"></span>
      </xsl:if>
      <!--bar-->
      <xsl:if test="$noticeCode='1126'">
        <xsl:comment>helper</xsl:comment>
        <span about="this:notifiableThing" property="honours:isAwarded" resource="this:bar" typeof="honours:Bar"></span>
        <span about="this:bar" property="honours:isAwardedTo" resource="this:person-1"></span>
        <span about="this:bar" property="honours:isAssociatedWithAward" resource="this:award-1"></span>
        <span about="this:award-1" property="rdf:type" resource="honours:{wlf:stripSpecialChars($bar)}"></span>      
        <span about="this:honour-1" property="honours:isMemberOfOrder" resource="honours:{$order}"></span>
      </xsl:if>
      <xsl:choose>
        <xsl:when test="$noticeCode = '2903'">
          ...
          <span resource="{$noticeURI}"
            typeof="personal-legal:DeceasedEstatesNotice personal-legal:WillsAndProbateNotice gaz:Notice"/>
  ...
  <xsl:template name="metadata">
    ...
    <dl class="metadata">
      ...
      <dd data-ui-class="category">
        <xsl:value-of
          select="$paramConfigXml/gzc:Configuration//gzc:Category[@Code = substring($noticeType,1,2)]/@Name"
        />
      </dd>
      <dt>Notice type:</dt>
      <dd data-ui-class="notice-type">
        <xsl:value-of
          select="$paramConfigXml/gzc:Configuration//gzc:Notice[@Code = $noticeType]/gzc:Name"/>
      </dd>
      <dt>Publication date:</dt>
      <dd about="{$noticeURI}" property="{$has-publication-date}" content="{$publishDate}"
      ...
      <xsl:if test="$noticeCode = '2903' and gz:Person/gz:DeathDetails/gz:NoticeOfClaimsDate">
        <dt>Claim expires:</dt>
        <dd about="this:notifiableThing" property="personal-legal:hasClaimDeadline"
          content="{gz:Person/gz:DeathDetails/gz:NoticeOfClaimsDate/@Date}" datatype="xsd:date">
  ...
  <xsl:template match="*:dt[@data-copy-title='true']"  mode="nameSecondPass">
    <dt>
      ...
      <xsl:value-of select=".//preceding-sibling::*:dt[@data-gazettes='custom-title'][1]"></xsl:value-of>
    </dt>
  </xsl:template> 
  ...
  <xsl:template match="*:dd[following-sibling::*[1][@data-gazettes='suffix']]"  mode="dlSecondPass">
    <dd>
      ...
      <xsl:if test="normalize-space($suffix_text)!=''">
        <xsl:text> </xsl:text>
        <span><xsl:value-of select="$suffix_text"/></span>
      </xsl:if>
    </dd>
  </xsl:template>
  <xsl:template match="*:dd[preceding-sibling::*[2][@data-gazettes='prefix']]"  mode="dlSecondPass">
    <dd>
      ...
      <xsl:if test="normalize-space($prefix_text)!=''">
        <span><xsl:value-of select="$prefix_text"/><xsl:text> </xsl:text></span>
      </xsl:if>
      <xsl:apply-templates select="node()"  mode="nameSecondPass"/>
    </dd>
  </xsl:template>
  ...
  <xsl:template match="gz:Surname" mode="person2903">
    <xsl:param name="personURI"/>
    <dt>Surname:</dt>
    <dd typeof="gaz:Person" about="{$personURI}" property="foaf:familyName"
      data-gazettes="Surname">
      <xsl:value-of select="."/>
    </dd>
  </xsl:template>
  <xsl:template match="gz:Forename" mode="person2903">
    <xsl:param name="personURI"/>
    <dt>First name:</dt>
    <dd typeof="gaz:Person" about="{$personURI}" property="foaf:firstName"
      data-gazettes="Forename">
      <xsl:value-of select="."/>
    </dd>
  </xsl:template>
  <xsl:template match="gz:MiddleNames" mode="person2903">
    <xsl:param name="personURI"/>
    <dt>Middle name(s):</dt>
    <dd typeof="gaz:Person" about="{$personURI}" property="foaf:givenName"
      data-gazettes="Forename">
      <xsl:value-of select="."/>
    </dd>
  </xsl:template>
  ...
  <xsl:template name="type2903">
    ...
    <xsl:if test="gz:Person/gz:DeathDetails/gz:Date">
      ...
      <dd property="personal-legal:dateOfDeath" typeof="gaz:Person" datatype="xsd:date"
        about="{$personURI}" content="{gz:Person/gz:DeathDetails/gz:Date/@Date}">
  ...
  <xsl:template match="@*|node()" mode="init">
    ...
    <xsl:variable name="list_formerly"
      select="('formerly known as','Formerly known as','formerly','Formerly')"/>
    ...
    <xsl:for-each select="$rawname">
      <xsl:choose>
        ...
        <xsl:when test="wlf:contains(.,$list_formerly)">
          ...
          <xsl:variable name="clean_formerly_text"
            select="normalize-space(wlf:replace-multi(.,$common_characters,$empty))"/>
        ...
        <xsl:when test="wlf:contains(.,$list_otherwise)">
          ...
          <xsl:variable name="clean_otherwise_text"
            select="normalize-space(wlf:replace-multi(.,$common_characters,$empty))"/>
          <!-- calling split function-->
        ...
        <xsl:when test="wlf:contains(.,$list_also)">
          ...
          <xsl:variable name="clean_also_text"
            select="normalize-space(wlf:replace-multi(.,$common_characters,$empty))"/>
          <!-- calling split function-->
        ...
        <xsl:when test="wlf:contains(.,$list_maiden)">
          ...
          <xsl:variable name="clean_maiden_text"
            select="normalize-space(wlf:replace-multi(.,$common_characters,$empty))"/>
          <!-- calling split function-->
    ...
    <xsl:for-each select="$text">
      <xsl:choose>
        ...
        <xsl:when test="wlf:contains(.,$list_formerly)">
          ...
          <xsl:variable name="formerly_text" select="wlf:replace-multi(.,$list_formerly,$empty)"/>
          <xsl:variable name="clean_formerly_text"
            select="normalize-space(wlf:replace-multi($formerly_text,$common_characters,$empty))"/>
            <xsl:call-template name="formerly">
        ...
        <xsl:when test="wlf:contains(.,$list_otherwise)">
          ...
          <xsl:variable name="otherwise_text" select="wlf:replace-multi(.,$list_otherwise,$empty)"/>
          <xsl:variable name="clean_otherwise_text"
            select="normalize-space(wlf:replace-multi($otherwise_text,$common_characters,$empty))"/>
            <xsl:call-template name="otherwise">
        ...
        <xsl:when test="wlf:contains(.,$list_also)">
          ...
          <xsl:variable name="also_text" select="wlf:replace-multi(.,$list_also,$empty)"/>
          <xsl:variable name="clean_also_text"
            select="normalize-space(wlf:replace-multi($also_text,$common_characters,$empty))"/>
            <xsl:call-template name="also">
        ...
        <xsl:when test="wlf:contains(.,$list_maiden)">
          ...
          <xsl:variable name="maiden_text" select="wlf:replace-multi(.,$list_maiden,$empty)"/>
          <xsl:variable name="clean_maiden_text"
            select="normalize-space(wlf:replace-multi($maiden_text,$common_characters,$empty))"/>
            <xsl:call-template name="maiden">
  ...
  <xsl:template match="gz:Notice/gz:Legislation">
    <h3 data-gazettes="Legislation">
      <span about="this:notifiableThing" property="gaz:isEnabledByLegislation"
        resource="{wlf:clean(wlf:serialize(.))}" typeof="leg:Legislation"
        data-gazettes="Legislation">
        <span about="{wlf:clean(wlf:serialize(.))}" property="rdfs:label"
          content="{wlf:serialize(.)}">
          <xsl:value-of select="upper-case(wlf:serialize(.))"/>
        </span>
      </span>
    </h3>
    ...
    <xsl:choose>
      ...
      <xsl:when test="count(gz:Text/text()) = 1 and starts-with(gz:Text/text(),'•')">
        <ul data-created-bullet="true" class="bullet-list">
          ...
          <li>
            <xsl:value-of
              select="normalize-space(replace(replace(replace(gz:Text/text(),'•',''),'&#x2002;',' '),'&#x2003;',' '))"
            />
          </li>
        </ul>
      </xsl:when>
      <xsl:when test="child::gz:AwardName">
        <xsl:choose>
          <xsl:when test="$noticeCode='1126'">
            <div data-gazettes="Award" property="honours:isAwarded" resource="this:bar" typeof="honours:{$bar}">
              <xsl:next-match/>
            </div>
          </xsl:when>
          <xsl:when test="$noticeCode=('1127','1130','1131','1132','1133','1134','1135','1136')">
          <div data-gazettes="Award" property="honours:isAwarded" resource="this:award" typeof="honours:{$medal}">
            <xsl:next-match/>
          </div>
          </xsl:when>
          <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="$noticeCode = (2432, 2443)">
        <p>
          ...
          <strong property="{$has-company-name}" datatype="xsd:string" data-gazettes="CompanyName"
            data-gazettes-class="{@Class}">
      ...
      <xsl:otherwise>
        <h3>
          <span property="{$has-company-name}" datatype="xsd:string" data-gazettes="CompanyName"
            data-gazettes-class="{@Class}">
            <xsl:value-of select="upper-case(.)"/>
          </span>
        </h3>
  ...
  <xsl:template match="gz:Notice/gz:CompanyGroup/gz:Legislation">
    <span about="this:notifiableThing" property="gaz:isEnabledByLegislation"
      resource="{wlf:clean(wlf:serialize(.))}" typeof="leg:Legislation" data-gazettes="Legislation">
  ...
  <xsl:template match="gz:Notice/gz:Company/gz:CompanyRegisteredOffice">
    <p property="{$has-registered-office}" resource="{wlf:compound-name(..,.)}"
      typeof="vcard:Address" data-gazettes="CompanyRegisteredOffice">
  ...
  <xsl:template match="gz:Notice/gz:Company/gz:PrincipalTradingAddress">
    <p property="{$has-principal-trading-address}" resource="{wlf:compound-name(..,.)}"
      typeof="vcard:Address" data-gazettes="PrincipalTradingAddress">
  ...
  <xsl:template match="gz:CompanyPrevious/gz:CompanyName">
    <span property="gazorg:previousCompanyName" data-gazettes="CompanyName"
      data-gazettes-class="{@Class}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:Notice/gz:Partnership">
    ...
    <div property="{$prop}" resource="{wlf:name-sibling(.)}" typeof="gazorg:Partnership"
      data-gazettes="Partnership">
      <xsl:apply-templates/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="gz:Notice/gz:Partnership/gz:PartnershipAddress">
    <p property="vcard:adr" resource="{wlf:compound-name(..,.)}" typeof="vcard:Address"
      data-gazettes="PartnershipAddress">
      <xsl:apply-templates/>
    </p>
  </xsl:template>
  ...
  <xsl:template match="gz:Notice/gz:Partnership/gz:PartnershipDetails">
    <p property="{$has-partnership-details}" datatype="xsd:string"
      data-gazettes="PartnershipDetails">
      <xsl:apply-templates/>
    </p>
  </xsl:template>

  <xsl:template match="gz:Notice/gz:Partnership/gz:PartnershipPrevious">
    <p property="{$has-previous-partnership}" datatype="xsd:string"
      data-gazettes="PartnershipPrevious">
  ...
  <xsl:template match="gz:Notice/gz:Court">
    <div property="{$has-court-case}" resource="this:courtCase" typeof="court:CourtCase"
      data-gazettes="Court">
      <xsl:apply-templates/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="gz:Notice/gz:Court/gz:CourtDistrict">
    ...
    <p>
      <span data-gazettes="CourtDistrict" resource="this:court" property="{$has-court-district}"
        datatype="xsd:string">
        <xsl:apply-templates/>
      </span>
      <xsl:text> </xsl:text>
  ...
  <xsl:template match="gz:Notice/gz:Court/gz:PetitionFilingDate">
    <p resource="this:notifiableThing" property="{$recognizes-petition-filing-date}"
      datatype="xsd:date" content="{@Date}" data-gazettes="PetitionFilingDate" data-date="{@Date}">
  ...
  <xsl:template match="gz:Notice/gz:Court/gz:WindingUpOrderDate">
    <p resource="this:notifiableThing" property="{$recognizes-winding-up-order-date}"
      datatype="xsd:date" content="{@Date}" data-gazettes="WindingUpOrderDate" data-date="{@Date}">
  ...
  <xsl:template match="gz:Notice/gz:Court/gz:VolWindingUpResolutionDate">
    <p about="this:notifiableThing" property="{$recognizes-vol-winding-up-resolution-date}"
      datatype="xsd:date" data-gazettes="VolWindingUpResolutionDate" data-date="{@Date}">
  ...
  <xsl:template match="gz:Notice/gz:Court/gz:BankruptcyOrderDate">
    ...
    <p about="this:notifiableThing" property="insolvency:dateOfBankruptcyOrder"
      content="{concat(@Date,concat('T',concat($time,':00')))}" datatype="xsd:dateTime"
      data-gazettes="BankruptcyOrderDate" data-date="{@Date}">
  ...
  <xsl:template match="gz:Notice/gz:Court/gz:BankruptcyRestrictions">
    <p about="this:notifiableThing" property="{$applies-bankruptcy-restrictions}"
      datatype="xsd:date" data-gazettes="BankruptcyRestrictions">
  ...
  <xsl:template match="gz:Notice/gz:Court/gz:TypeOfPetition">
    <p resource="this:notifiableThing" property="insolvency:typeOfPetition" datatype="xsd:string"
      content="{wlf:serialize(.)}" data-gazettes="TypeOfPetition">
  ...
  <xsl:template match="gz:Notice/gz:Society/gz:SocietyName">
    <p>
      <span about="{wlf:name(.)}" property="{$has-society-name}" datatype="xsd:string"
        data-gazettes="SocietyName">
        <xsl:apply-templates/>
      </span>
    </p>
  </xsl:template>
  ...
  <xsl:template match="gz:Notice/gz:Society/gz:SocietyNumber">
    <p>
      <span about="{wlf:name(../gz:SocietyName)}" property="{$has-society-number}"
        datatype="xsd:string" data-gazettes="SocietyNumber">
        <xsl:apply-templates/>
      </span>
    </p>
  </xsl:template>
...
<xsl:template name="insidepara">
  ...
  <xsl:template match="gz:Notice/gz:Person">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <div property="{$prop}" resource="{if ($noticeCode = '2903') then 'this:deceasedPerson' else wlf:name-sibling(.)}" typeof="person:Person"
          data-gazettes="Person">
        <!--  <xsl:if test="$noticeCode='1131'"><xsl:attribute name="about">this:person-1</xsl:attribute></xsl:if>-->
          <xsl:apply-templates/>
          <!--<xsl:apply-templates select="following-sibling::gz:Citation" mode="persondiv"/>-->
        </div>
  ...
  <xsl:template
    match="gz:Notice/gz:PersonName[not(@Class='Administrator')] | gz:Notice/gz:Text/gz:PersonName[not(@Class='Administrator')]">
    <xsl:variable name="prop">
      <xsl:choose>
    ...
    <div property="{$prop}" resource="{wlf:name-sibling(.)}" typeof="person:Person"
      data-gazettes="Person">
  ...
  <xsl:template match=" gz:Notice/gz:P/gz:Text/gz:PersonName[not(@Class='Administrator')]">
    ...
    <span property="{$prop}" resource="{wlf:name-sibling(.)}" typeof="person:Person"
      data-gazettes="Person">
      <!-- &#160; -->
  ...
  <xsl:template match="gz:Notice/gz:Person/gz:PersonName/gz:Title">
    <span data-gazettes="Title" property="foaf:title"><xsl:apply-templates/></span>
    <xsl:text> </xsl:text>
  </xsl:template>
  <xsl:template match="gz:Notice/gz:Person/gz:PersonName/gz:PostNominal">
    <span data-gazettes="PostNominal" property="person:honour"><xsl:apply-templates/></span>
    <xsl:text> </xsl:text>
  </xsl:template>
  <xsl:template match="gz:Notice/gz:Person/gz:PersonAddress">
    <p typeof="vcard:Address" property="person:hasAddress" resource="{wlf:compound-name(..,.)}"
      data-gazettes="PersonAddress">
      <xsl:apply-templates/>
    </p>
  </xsl:template>

  <xsl:template match="gz:Notice/gz:Person/gz:PersonAddress/gz:AddressLineGroup">
    <span property="vcard:label" content="{wlf:serialize(.)}" data-gazettes="AddressLineGroup"
      data-gazettes-class="{@Class}"/>
    <xsl:apply-templates/>
  </xsl:template>

  <xsl:template match="gz:Notice/gz:Person/gz:PersonStatus">
    <p about="{wlf:name-sibling(..)}" property="person:hasStatus" datatype="xsd:string"
      data-gazettes="PersonStatus">
      <xsl:apply-templates/>
    </p>
  </xsl:template>
  ...
  <xsl:template match="gz:OccupationPosition">    
      <xsl:choose>
        <xsl:when test="contains(.,',')">
          ...
          <xsl:text>, </xsl:text>
          <span about="this:occupation-1" property="gazorg:isMemberOfOrganisation" resource="this:organisation-1"><xsl:value-of select="substring-after(.,',')"/></span>         
        </xsl:when>
        <xsl:otherwise>         
          <span about="this:occupation-1" property="gazorg:isMemberOfOrganisation" resource="this:organisation-1"><xsl:apply-templates/></span>      
        </xsl:otherwise>
      </xsl:choose>
 ...
 <xsl:template match="gz:Notice/gz:Person/gz:Department" mode="notice_1122">
   <span about="this:department-1" typeof="gazorg:Department" property="gazorg:departmentName"><xsl:apply-templates/></span>
  ...
  <xsl:template match="gz:Notice/gz:Person/gz:PersonName/gz:Rank">
    <xsl:choose>
      <xsl:when test="$noticeCode='1135'">
        <span data-gazettes="Rank" property="military:hasRank"><xsl:apply-templates/></span>
      </xsl:when>
      <xsl:otherwise>
 ...
 <!-- 1131-->
  ...
  <xsl:template match="gz:ServiceBranch">
    <xsl:choose>
      <xsl:when test="$noticeCode='1135'"><span property="honours:inSubDivision" resource="honours:{concat('Royal',wlf:capitalize-first(lower-case(.)))}"><xsl:apply-templates/></span></xsl:when>
      <xsl:otherwise/>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="gz:Notice/gz:Honour">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$noticeCode =('1132','1133','1134','1135','1136')">
        <span property="honours:isAwarded" resource="this:medal-1" typeof="honours:{$medal}"><xsl:apply-templates/></span>
      </xsl:when>
      <xsl:otherwise>
        <span property="honours:isAwarded" resource="this:award" typeof="honours:{wlf:stripSpecialChars($honour)}"><xsl:apply-templates/></span>
      </xsl:otherwise>
    </xsl:choose>  
    <xsl:text> </xsl:text>
    </p>
  ...
  <xsl:template match="gz:Notice/gz:Division">
    <div data-gazettes="Division" property="honour:inDivision" resource="honours:{wlf:stripSpecialChars(.)}">
      <xsl:apply-templates/>
    </div>
  </xsl:template>  
  <xsl:template match="gz:Notice/gz:SubDivision">
    <div data-gazettes="Category" property="honour:inSubDivision" resource="honours:{wlf:stripSpecialChars(.)}">
      <xsl:apply-templates/>
    </div>
  </xsl:template>
  
  <xsl:template match="gz:Notice/gz:Region">
    <span property="honours:inRegion" ><xsl:apply-templates/></span>
  </xsl:template>
  ...
  <xsl:template match="gz:Notice/gz:Person/gz:BirthDetails">
    ...
    <p about="{wlf:name-sibling(..)}" property="person:dateOfBirth" datatype="xsd:date"
      content="{gz:Date/@Date}" data-gazettes="BirthDetails">
  ...
  <xsl:template match="gz:Notice/gz:Person/gz:DeathDetails">
    ...
    <p about="{wlf:name($person-name)}" property="{$has-death-details}" datatype="xsd:string"
      content="{normalize-space($death-details)}" data-gazettes="DeathDetails">
  ...
  <xsl:template match="gz:Notice/gz:Administration/gz:AdditionalContact/gz:PersonName">
    <span property="person:hasAdditionalContact" datatype="xsd:string"
      content="{wlf:serialize-name(if(*) then * else .)}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>

  <xsl:template match="gz:Notice/gz:Administration/gz:OfficeHolderCapacity">
    <xsl:variable name="previous-administrator"
      select="wlf:name-sibling(preceding::gz:Administrator)"/>
    <p data-gazettes="OfficeHolderCapacity">
      <xsl:choose>
        <xsl:when test="$noticeCode = '2443'"/>
        <xsl:otherwise>Capacity of office holder(s): </xsl:otherwise>
      </xsl:choose>
      <span about="{$previous-administrator}" property="person:hasRole"
        resource="{concat($previous-administrator,'-role')}">
        <span resource="{concat($previous-administrator,'-role')}" typeof="person:Role"
          property="person:roleName" datatype="xsd:string">
          <xsl:apply-templates/>
        </span>
      </span>
    </p>
  ...
  <xsl:template match="gz:DateOfAppointment">
    <p>
      ...
      <span property="{$has-date-of-appointment}" datatype="xsd:date" content="{@Date}"
        data-gazettes="DateOfAppointment" data-date="{@Date}">
        <xsl:apply-templates/>
      </span>
    </p>
  </xsl:template>
  ...
  <xsl:template match="gz:DateSigned">
    <p data-gazettes="DateSigned">
      <span property="{$signed-on-date}" datatype="xsd:date" content="{@Date}"
        data-gazettes="DateSigned" data-date="{@Date}">
        <xsl:apply-templates/>
      </span>
    </p>
  </xsl:template>
  ...
  <xsl:template
    match="gz:Text/gz:AddressLineGroup[@Class='Administrator'] | gz:Administration/gz:Administrator/gz:AddressLineGroup">
	<xsl:choose>
		<xsl:when test="$noticeCode = 2903">
  ...
  <xsl:template
    match="gz:Administration/gz:Administrator/gz:PersonName | gz:Text//gz:PersonName[@Class='Administrator']">
    <span
      property="{if (../following-sibling::*[1][self::gz:DateSigned]) then $has-authorising-person 
        ...
        resource="{ if ($noticeCode=('2903') and parent::*[name() != 'Administrator']) then 'this:deceasedPerson' else if ($noticeCode=('2903') and parent::*[name() = 'Administrator']) then 'this:estateExecutor' else if (../following-sibling::*[1][self::gz:DateSigned])  then 'this:signatory-1' else wlf:compound-name(..,.)}"
      typeof="{ if (../following-sibling::*[1][self::gz:DateSigned]) then 'person:Person' else 'foaf:Agent'}"
      data-gazettes="{@Class}">
      ...
      <xsl:if test="../gz:OfficeHolderNumber[1]">
        <xsl:variable name="previous-siblings"
          select="(count(preceding-sibling::gz:PersonName) + 1)" as="xs:integer"/>
        <span property="person:hasIPnum" datatype="xsd:string"
          content="{../gz:OfficeHolderNumber[$previous-siblings]/text()}"/>
      </xsl:if>
      <xsl:if test="../gz:OfficeHolderCapacity[1]">
        <span property="person:hasIPCapacity" datatype="xsd:string"
          content="{../gz:OfficeHolderCapacity/text()}"/>
      </xsl:if>
      <xsl:if test="../gz:AddressLineGroup[1]">
        <span property="vcard:adr"
          resource="{wlf:compound-name(..,following-sibling::gz:AddressLineGroup[1])}"/>
      </xsl:if>
      <xsl:if test="../gz:FirmName[1]">
        <span property="gazorg:hasOrganisationMember"
          resource="{wlf:compound-name(..,following-sibling::gz:FirmName[1])}"/>
      </xsl:if>
      <xsl:choose>
  ...
  <xsl:template match="gz:Text/gz:AddressLineGroup[@Class='Company']">
    <span about="this:company-1" property="vcard:adr" resource="this:company-1-address-0"
      data-gazettes="AddressLineGroup" data-gazettes-class="{@Class}">
      <span about="this:company-1-address-0" typeof="vcard:Address" property="vcard:label"
        content="{wlf:serialize(.)}">
        <xsl:apply-templates/>
      </span>
    </span>
  ...
  <xsl:template match="gz:Text/gz:AddressLineGroup[@Class='Meeting']">
    <span about="this:notifiableThing" property="insolvency:meetingLocation"
      resource="this:meetingAddress-0" data-gazettes="AddressLineGroup"
      data-gazettes-class="{@Class}">
      <span about="this:meetingAddress-0" typeof="vcard:Address" property="vcard:label"
        content="{wlf:serialize(.)}">
        <xsl:apply-templates/>
      </span>
    </span>
  ...
  <xsl:template match="gz:Text/gz:AddressLineGroup[@Class='Petitioner']">
    <span about="this:petitioner-1" property="vcard:adr" resource="this:petitioner-1-address-0"
      data-gazettes="AddressLineGroup" data-gazettes-class="{@Class}">
      <span about="this:petitioner-1-address-0" typeof="vcard:Address" property="vcard:label"
        content="{wlf:serialize(.)}">
        <xsl:apply-templates/>
      </span>
    </span>
  ...
  <xsl:template match="gz:Text/gz:AddressLineGroup[@Class='Court']">
    <xsl:choose>
      <xsl:when test="$noticeCode = ('2450','2451','2461')">
        <span about="this:notifiableThing" property="insolvency:placeOfHearing"
          resource="this:hearingAddress" data-gazettes="AddressLineGroup"
          data-gazettes-class="{@Class}">
          <span about="this:hearingAddress" typeof="vcard:Address" property="vcard:label"
            content="{wlf:serialize(.)}">
            <xsl:apply-templates/>
          </span>
        </span>
      ...
      <xsl:otherwise>
        <span about="this:court" property="vcard:adr" resource="this:court-address-0"
          data-gazettes="AddressLineGroup" data-gazettes-class="{@Class}">
          <span about="this:court-address-0" typeof="vcard:Address" property="vcard:label"
            content="{wlf:serialize(.)}">
            <xsl:apply-templates/>
          </span>
        </span>
  ...
  <xsl:template match="gz:Text/gz:AddressLineGroup[@Class='Person']">
    <span property="gaz:hasPerson" resource="person-1" typeof="person:Person"
      data-gazettes="AddressLineGroup" data-gazettes-class="{@Class}">
  ...
  <xsl:template
    match="gz:Text/gz:AddressLineGroup[(@Class!='Company' and @Class!='Petitioner' and @Class!='Court' and @Class!='Administrator' and @Class!='Meeting' and @Class!='Person' and @Class!='Partnership') or not(@Class)]">
    <span about="{wlf:clean(wlf:serialize(.))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="AddressLineGroup" data-gazettes-class="{@Class}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
      ...
      <xsl:when test="$noticeCode='1129' or $noticeCode='1132' or $noticeCode='1133' or $noticeCode='1134'">
        <span about="this:organisationAddress" data-gazettes="AddressLine" typeof="vcard:Address">
          <xsl:choose>
            ...
            <xsl:otherwise>
              <span  property="vcard:extended-address"><xsl:apply-templates/></span>
            </xsl:otherwise>
          </xsl:choose>          
        </span>
      </xsl:when>
  ...
  <xsl:template match="gz:AddressLineGroup/gz:country" mode="addressline">
    <span  property="vcard:country-name"><xsl:apply-templates/></span>
  </xsl:template>
  ...
  <xsl:template match="gz:AlsoKnownAs">
    <span resource="{wlf:name-sibling(..)}" property="person:alsoKnownAs"
      data-gazettes="AlsoKnownAs" datatype="xsd:string">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:Date[@Class='Hearing']">
    <span property="insolvency:dateOfHearing" datatype="xsd:dateTime"
      content="{concat(@Date,'T00:00:00')}" data-gazettes="Date" data-gazettes-class="{@Class}"
      data-date="{@Date}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:Date[@Class='Presentation']">
    <span property="insolvency:dateOfPetitionPresentation" datatype="xsd:date" content="{@Date}"
      data-gazettes="Date" data-gazettes-class="{@Class}" data-date="{@Date}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>

  <xsl:template match="gz:Date[@Class='Resolution']">
    <span property="insolvency:dateOfResolution" datatype="xsd:date" content="{@Date}"
      data-gazettes="Date" data-gazettes-class="{@Class}" data-date="{@Date}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:Date">
    <span property="gaz:relatedDate" content="{@Date}" datatype="xsd:date" data-gazettes="Date"
      data-gazettes-class="{@Class}" data-date="{@Date}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:DateOfAnnulment">
    ...
    <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="DateOfAnnulment" data-date="{@Date}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:DateOfDischarge">
    ...
    <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="DateOfDischarge" data-date="{@Date}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:DateOfCertificateOfDischarge">
    ...
    <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="DateOfCertificateOfDischarge" data-date="{@Date}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:FirmName">
    <xsl:choose>
      <xsl:when test="ancestor::gz:Administration[1][gz:DateSigned] and $noticeCode != '2903'">
        <span property="gazorg:hasOrganisationMember"
          resource="{concat('this:authoriser-',wlf:name-sibling-no-namespace(.))}"
      ...
      <xsl:when
        test="ancestor::gz:Text and count(preceding-sibling::gz:PersonName[@Class='Administrator']) &lt; 1 and count(following-sibling::gz:PersonName[@Class='Administrator']) &lt; 1">
        <span
          property="{if(@Class = 'Receiver') then $has-official-receiver
				else if(@Class = 'Administrator' and $noticeCode = ('2446','2431','2432','2460','2454','2435','2406','2408','2403','2443','2455','2433')) then $has-liquidator
				else if(@Class = 'Administrator' and $noticeCode = ('2413','2410','2411','2412','2445','2442')) then $has-administrator
				else if(@Class = 'Administrator' and $noticeCode = ('2423','2422')) then $has-receiver
				else if(@Class = 'Administrator' and $noticeCode = ('2503','2504','2509','2510')) then $has-official-receiver
				else if(@Class = 'Administrator' and $noticeCode = '2441') then $has-resolutionLiquidator else @Class}"
          resource="{concat('this:IP-',wlf:name-sibling-no-namespace(.))}" typeof="{'foaf:Agent'}"
          data-gazettes="{@Class}">
          <xsl:if test="../gz:OfficeHolderNumber[1]">
            <xsl:variable name="previous-siblings"
              select="(count(preceding-sibling::gz:PersonName) + 1)" as="xs:integer"/>
          ...
          <span property="gazorg:hasOrganisationMember" resource="{wlf:compound-name(..,.)}"
            typeof="gazorg:ForProfitOrganisation" data-gazettes="FirmName">
            <span property="gazorg:name">
              <xsl:apply-templates/>
            </span>
    	...
    	<xsl:when test="$noticeCode='2903'">
    		<span typeof="foaf:Agent" about="this:estateExecutor"
    			data-gazettes="FirmName">
      ...
      <xsl:otherwise>
        <!--<span about="{concat('this:text-',wlf:name-sibling-no-namespace(.))}"
      	...
      	<span about="{wlf:compound-name(..,.)}" typeof="gazorg:ForProfitOrganisation"
      		data-gazettes="FirmName">
      		<span property="gazorg:name">
      			<xsl:apply-templates/>
      		</span>
  ...
  <xsl:template match="gz:GroundsOfAnnulment">
    ...
    <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="GroundsOfAnnulment">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:Legislation">
    <span resource="this:notifiableThing"/>
    <span property="{$is-enabled-by-legislation}" resource="{wlf:clean(wlf:serialize(.))}"
      typeof="leg:Legislation" data-gazettes="Legislation">
  ...
  <xsl:template match="gz:NoticeOfClaims">
    ...
    <span resource="{wlf:clean(wlf:serialize(.))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="NoticeOfClaims">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:OfficeHolderCapacity">
    <xsl:variable name="previous-administrator"
      select="wlf:compound-name(..,preceding-sibling::gz:PersonName[last()])"/>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <span about="{$previous-administrator}" property="person:hasRole" typeof="person:Role"
          resource="{concat($previous-administrator,'-role')}" data-gazettes="OfficeHolderCapacity">
          <span property="person:roleName" datatype="xsd:string">
            <xsl:apply-templates/>
          </span>
        </span>
  ...
  <xsl:template match="gz:Petitioner">
    <p about="this:notifiableThing" property="insolvency:hasPetitioner"
      resource="{wlf:name-sibling(.)}" typeof="foaf:Agent" data-gazettes="Petitioner">
      <xsl:apply-templates/>
    </p>
  </xsl:template>
  ...
  <xsl:template match="gz:RelatedCase">
    ...
    <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="RelatedCase">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:Retraction">
    <p class="retraction" data-gazettes="Retraction">
      <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
        content="{wlf:serialize(.)}" data-gazettes="Retraction">
        <xsl:apply-templates/>
      </span>
    </p>
  </xsl:template>
  ...
  <xsl:template match="gz:Salutation">
    <span resource="{wlf:clean(wlf:serialize(..))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="Salutation">
      <xsl:apply-templates/>
    </span>
    <!-- &#160; -->
  </xsl:template>
  ...
  <xsl:template match="gz:Substitution">
    <p class="substitution" data-gazettes="Substitution">
      <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
        content="{wlf:serialize(.)}" data-gazettes="Substitution">
        <xsl:apply-templates/>
      </span>
    </p>
  </xsl:template>
  ...
  <xsl:template match="gz:Telephone">
    <span property="loc:phoneNumber" content="{wlf:serialize(.)}" data-gazettes="Telephone"
      data-gazettes-class="{@Class}">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
    ...
    <xsl:choose>
      <xsl:when test="@Class='Meeting'">
        <xsl:if test="root()//gz:Date[@Class = 'Meeting']">
          <span about="this:notifiableThing"
            property="{concat('insolvency:meetingTime',if(count(preceding-sibling::gz:Time) = 0) then '' else count(preceding-sibling::gz:Time) + 1)}"
            datatype="xsd:dateTime"
            content="{if (count(parent::*/gz:Date[@Class = 'Meeting']) = 1) then concat(parent::*/gz:Date[@Class = 'Meeting']/@Date,'T',@Time)
          ...
          else concat(following-sibling::gz:Date[@Class = 'Meeting'][1]/@Date,'T',@Time)}"
          > </span>
        </xsl:if>
        <xsl:apply-templates/>
      </xsl:when>
  ...
  <xsl:template match="gz:Notice/gz:TradingAs">
    <p resource="{wlf:name-sibling(../*[1])}" property="gazorg:isTradingAs"
      content="{wlf:serialize(.)}" data-gazettes="TradingAs">
  ...
  <xsl:template match="gz:TradingDetails">
    ...
    <span resource="{wlf:clean(wlf:serialize(../*[1]))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="TradingDetails">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:TradingPrevious">
    ...
    <span property="gazorg:previousCompanyName" datatype="xsd:string"
      data-gazettes="TradingPrevious">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:TradingAddress">
    ...
    <span resource="{wlf:clean(wlf:serialize(.))}" property="gzw:has{name()}"
      content="{wlf:serialize(.)}" data-gazettes="TradingAddress">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
  ...
  <xsl:template match="gz:TypeOfPetition">
    ...
    <span resource="this:notifiableThing" property="insolvency:typeOfPetition" datatype="xsd:string"
      data-gazettes="TypeOfPetition">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
    ...
    <img>
      ...
      <xsl:attribute name="src">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of
              select="concat(substring-before(normalize-space(.),'.'),'-orig.',substring-after(normalize-space(.),'.'))"
            />
          </xsl:otherwise>
        </xsl:choose>
      </xsl:attribute>
  ...
  <xsl:template name="gzImgAttrs">
    ...
    <xsl:variable name="vWidthAttributeVal" select="normalize-space($pWidthAttribute)"
      as="xs:string?"/>
    <xsl:variable name="vHeightAttributeVal" select="normalize-space($pHeightAttribute)"
      as="xs:string?"/>
    ...
    <xsl:variable name="vWidth" as="xs:string?">
      <xsl:choose>
       ...
       for now we will  smuggle them through as class attribute -->
        <xsl:when
          test="$vWidthAttributeVal = ('auto', 'scale-to-fit', 'fit-page-to-image', 'spread')">
    ...
    <xsl:variable name="vHeight" as="xs:string?">
      <xsl:choose>
        ...
        <!-- At some point we may wish to set these values to particular pixel sizes -->
        <xsl:when
          test="$vHeightAttributeVal = ('auto', 'scale-to-fit', 'fit-page-to-image', 'spread')">
  ...
  <xsl:template match="gz:Tabular">
    <xsl:choose>
      ...
      <xsl:when
        test="html:table[@cols='1']/html:tbody[count(*) = 1]/html:tr[count(*) = 1]/html:td[count(*) = 1]/gz:Image">

        <figure>
          <!-- If a the Image has a paragraph of text immediately preceeding it then make this the caption -->
          <xsl:if
            test="local-name(preceding-sibling::*[1]) = 'P' and preceding-sibling::gz:P[1]/.[count(*) = 1]">
            <figcaption>
  ...
  <xsl:template
    match="gz:P[count(*) = 1 and (local-name(following-sibling::*[1]) = 'Tabular') and following-sibling::*[1]/html:table[@cols='1']/html:tbody[count(*) = 1]/html:tr[count(*) = 1]/html:td[count(*) = 1]/gz:Image]"/>
      ...
      <xsl:for-each select="html:col">
        <col>
          ...
          <xsl:attribute name="width"><xsl:value-of
              select="round((100 div $totalwidth) * $thiscolumn)"/>%</xsl:attribute>
        </col>
      </xsl:for-each>
    </colgroup>
  </xsl:template>
...
returns integer set: (2,4,5,6)
  ...
  <xsl:function name="wlf:get2Digits">
    <xsl:param name="arg"/>
    <xsl:value-of
      select="if ($arg castable as xs:integer and xs:integer($arg) &lt; 10) then concat('0',$arg) else $arg"
    />
  </xsl:function>
