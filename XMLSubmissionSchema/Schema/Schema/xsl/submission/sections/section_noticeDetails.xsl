...
http://www.nationalarchives.gov.uk/doc/open-government-licence/-->
  ...
  <xsl:template match="section[@name='noticeDetails']" mode="RDFa">
    ...
    <span about="http://www.thegazette.co.uk/id/notice/{$nId}" property="dc11:publisher" content="TSO (The Stationery Office), St Crispins, Duke Street, Norwich, NR3 1PD, 01603 622211, customer.services@tso.co.uk"/>			
  ...
  <xsl:template match="section[@name='noticeDetails']" mode="content">
    ...
    <section id="{@name}">
      ...
      <dl>
        ...
        <dd about="http://www.thegazette.co.uk/id/notice/{$nId}" property="gaz:hasNoticeCode" datatype="xsd:integer"><xsl:value-of select="../@notice-type"/></dd>
        <dt>Notice number:</dt>
        <dd about="http://www.thegazette.co.uk/id/notice/{$nId}" property="gaz:hasNoticeNumber" datatype="xsd:integer">1019636</dd>
        <dt>Publication date:</dt>
        <dd about="http://www.thegazette.co.uk/id/notice/{$nId}" content="{earliestPublishDate}" datatype="xsd:date" property="gaz:hasPublicationDate"><time datetime="{earliestPublishDate}"><xsl:value-of select="earliestPublishDate"/></time></dd>
        ...
        <xsl:if test="//form/@noticeTypeCode = '2903'">
          <dt>Claim expires:</dt>
          <dd><time about="this:notifiableThing" content="{claimDeadlineDate}" datatype="xsd:date" datetime="{claimDeadlineDate}" property="gaz:hasClaimDeadline"><xsl:value-of select="claimDeadlineDate"/></time></dd>
        </xsl:if>
      </dl>
    </section>
  </xsl:template>
